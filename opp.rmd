...
#load the data
ga <- read_csv("~/Desktop/Codes/pa_philadelphia_2019_02_25.csv")
```
...
View(ga) #to view the data table
str(ga)  #to see the characteristics of variables
glimpse(ga) #to see a short summary of values in each column
colnames(ga) #to view column headers
ga <- ga %>% mutate(subject_sex=as.factor(subject_sex)) #convert chr to factor levels for the variable `subject_sex`
ga <- ga %>% mutate(subject_race=as.factor(subject_race)) #convert chr to factor levels for the variable `subject_race`
```
...
a <- select(ga,subject_race, subject_age, subject_sex, type: contraband_found, arrest_made) #to select the subject_race columns and assign it to object a.
a <- ga %>% select(subject_race) #a different way to do the selection using %>% 
...
```{r bar-chart}
#data prep
barprep <- ga %>% group_by(subject_race) %>% 
  summarize(value=n())
...
pieprep <-ga %>% group_by( subject_race) %>%
  summarize(value=n()) %>%
  arrange(-value)
...
donut <- ga %>% group_by(subject_race) %>% 
  summarise(value=n()) %>%
plot_ly(labels=~subject_race, values=~value, showlegend=F) %>%  add_pie(hole=.5) %>% layout(title="Stopped Drivers by Race")
donut
```
...
ggplot(philly) + geom_polygon(aes(x=long, y=lat, group=group))+ coord_fixed(1.3)+
